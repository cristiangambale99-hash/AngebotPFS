...
// api/vertrag-pdf.js
//
// Liefert der Verwaltung den unterschriebenen Reinigungsvertrag als PDF.
//
//   GET /api/vertrag-pdf?id=<Auftragsnummer>
//   -> { ok:true, dateiname:"...", am:"...", pdf:"<base64>" }
//
// Nur fuer angemeldete Personen: Der Vertrag enthaelt Unterschrift und
// Adresse der Kundschaft. Die Kundschaft selbst bekommt ihr Exemplar per
// Mail; hier geht es um den internen Zugriff aus dem Fall heraus.

const SAMMLUNG = 'vertraege';
...
export default async function handler(req, res) {
  ...
  if (nutzer === null) return res.status(401).json({ ok: false, error: 'Nicht angemeldet.' });

  try {
    const id = String((req.query && req.query.id) || '');
    if (!id) return res.status(400).json({ ok: false, error: 'Keine Auftragsnummer übergeben.' });

    const eintrag = await lesen(SAMMLUNG, id);
    if (!eintrag || !eintrag.pdf) {
      return res.status(404).json({ ok: false, grund: 'kein-vertrag' });
    }

    return res.status(200).json({
      ok: true,
      dateiname: eintrag.dateiname || ('Reinigungsvertrag_' + id + '.pdf'),
      am: eintrag.am || '',
      pdf: eintrag.pdf
    });

  } catch (err) {
    console.error('vertrag-pdf.js', err);
    return res.status(500).json({ ok: false, error: err.message });
  }
}

/* ---- Anmeldung prüfen (gleiches Verfahren wie in orders.js) ---- */
function sitzungPruefen(req){
  try{
    const geheim = process.env.SESSION_SECRET;
    if(!geheim) return 'nicht-eingerichtet';
    let token = '';
